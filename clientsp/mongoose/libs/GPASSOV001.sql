...
-- -----------------------------------------------------
-- Table `GPASSO`.`PGGR004MB`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `GPASSO`.`PGGR004MB` ;

CREATE TABLE IF NOT EXISTS `GPASSO`.`PGGR004MB` (
  `_ID` INT(10) NOT NULL,
  `MKR_ID` INT(10) NULL,
  `DT_CREATED` DATETIME NULL,
  `ATH_ID` INT(10) NULL,
  ...
  PRIMARY KEY (`_ID`),
  CONSTRAINT `RF_PAGE005MT_PAGE_IDS`
    FOREIGN KEY (`PAGE_IDS`)
    REFERENCES `GPASSO`.`PAGE005MT` (`_ID`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION);

CREATE INDEX `RF_PAGE005MT_PAGE_IDS_idx` ON `GPASSO`.`PGGR004MB` (`PAGE_IDS` ASC);
...
CREATE TABLE IF NOT EXISTS `GPASSO`.`ROLE003MT` (
  ...
  CONSTRAINT `RK_PGGR004MB_PAGE_GRP_IDS`
    ...
    REFERENCES `GPASSO`.`PGGR004MB` (`_ID`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
...
CREATE INDEX `RK_ROLE003MT_ROLE_IDS_idx` ON `GPASSO`.`PRTL002MB` (`ROLE_IDS` ASC);

CREATE INDEX `RK_PGGR004MB_PAGE_GPR_IDS_idx` ON `GPASSO`.`PRTL002MB` (`PAGE_GRP_IDS` ASC);
